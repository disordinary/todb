...
This project originialy started as an experiment to create a binary dependency free index for the search-index search engine and kind of grew, there are other datbases out there written in node but they either have binary dependencies, require the keys to be in memory or the entire database. The goal of todb is to have a reasonably performant database and storage engine written in pure javascript and stores indexes on disk.

Why todb? Well if you squint just right, and have had a few drinks, then it kind of looks like todo - as in I'm doing this whenver the mood strikes me and it's likely always going to have things todo. Not to be confused with toodb which is a similar database that I'm writing in rust.
...
This project is a work in progress, some of its messy, some of it doesn't work properly, it's an experiment at this stage.
...
3. ACID compliant. (It actually probably is ACID compliant, it always commits to disk before firing the success callback.)
...
5. Joins.
6. Suited to the node ecosystem with streams, evented, etc. Use proxys and decorators as below
7. Rollbacks.
8. ACID compliance.
...
2. Data compaction ✓
3. Indexing (partialy completed, new records are indexed but it doesn't retrospectively add existing.)
4. Abstract LevelDown support
5. Streaming
6. Batch processing
7. Streaming merges
8. Splitting files for larger datasets
9. Data querying (kind of, you can query an indexed field, but the goal is to be able to query the entire object and also join together data.)
